...
export type GesprekBeurt = { rol: string; teks: string };
export type PortefeuljeItem = { simbool: string; aantal: number; koopprys: number; geldeenheid?: string };
...
/** Die VRA BUITELYN-agent as herbruikbare funksie — web-chat én Telegram-bot. */
export async function beantwoordMarkteVraag(
  geskiedenis: GesprekBeurt[],
  opsies?: { portefeulje?: PortefeuljeItem[]; ekstraInstruksies?: string }
): Promise<string> {
  const portKonteks =
    opsies?.portefeulje && opsies.portefeulje.length
      ? `\n\nGebruiker se portefeulje (blaaier-gestoor): ${opsies.portefeulje
          .map((b) => `${b.simbool} × ${b.aantal} teen ${b.koopprys} ${b.geldeenheid ?? "ZAR"} per aandeel`)
          .join("; ")}`
      : "";
  const ekstra = opsies?.ekstraInstruksies ? `\n\n${opsies.ekstraInstruksies}` : "";
  ...
  for (let stap = 0; stap < MAX_STAPPE; stap++) {
    const res = await client.responses.create({
      model: MODEL,
      instructions: STELSEL + portKonteks + ekstra,
      input,
      tools: TOOLS,
  ...
  return "Te veel stappe — probeer 'n eenvoudiger vraag.";
}
