...
export async function GET() {
  ...
  const { data } = await service()
    ...
    .select("chat_id, koppel_kode, kode_verval, oggend, middag, aand, skuiwers")
    .eq("user_id", user.id)
    .maybeSingle();
  ...
  return NextResponse.json({
    ...
    voorkeure: data
      ? { oggend: data.oggend, middag: data.middag, aand: data.aand, skuiwers: data.skuiwers }
      : null,
  });
}
...
export async function PATCH(request: NextRequest) {
  ...
  for (const v of ["oggend", "middag", "aand", "skuiwers"] as const) {
