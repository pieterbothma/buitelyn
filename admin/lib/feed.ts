...
/* ── Admin extras: full post text for TTS ────────────────────────────── */

export type PostMetTeks = Post & { teks: string };

export function parseFeedMetTeks(xml: string): PostMetTeks[] {
...
/** Strip HTML to a readable plain text suitable for TTS. */
export function htmlNaLeesbareTeks(html: string): string {
  return html
    .replace(/<(script|style)[\s\S]*?<\/\1>/gi, " ")
    .replace(/<br\s*\/?>/gi, "\n")
    .replace(/<\/(p|h[1-6]|li|div|figure)>/gi, "\n\n")
    .replace(/<[^>]+>/g, " ")
    .replace(/&amp;/g, "&")
