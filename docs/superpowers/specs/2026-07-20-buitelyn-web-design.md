...
2. **Hero** — grid `[1.05fr_1fr]` md+: left = display headline (League Spartan 800, tight tracking) "Buitelyn." + red accent line "Ons trek die lyne." + tagline paragraph (from the feed's channel description) + CTAs ("Lees die nuutste uitgawe →" → newest post URL; "Teken in op Substack"). Right = `presenters-duo.png` cutout on off-white square, ringed by absolutely-positioned floating chips at slight rotations (red dot disc; `JSE ▲ +1,2%` ticker chip; `MARKTE`, `POLITIEK`, `SPORT` section tags; `EERSTE UITGAWE` microtag). Mobile: chips in a row beneath the portrait.
